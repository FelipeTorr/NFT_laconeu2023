classdef Simulator
    ...
    methods
        ...
        function obj=solve(obj,seed,V,Q,phi,previousV,currentQ)
			...
			for n=1:ceil(obj.finalTime*1/obj.h)
				...
				%Generate Noise
				[backgroundNoise,noiseFlags]=obj.stimulator.generateBackgroundNoise(noiseFlags);
				%Generate the stim
                ...
                if obj.plasticityFlag==2
                 ...
                 if mod(currentTime,5)==0
                    fprintf('Transcurred: %.2f seconds of %s at %s \n',currentTime,obj.monitor.filename,datestr(now,'HH:MM:SS DD-mm-YY'));
        ...
        function obj=solveAAS(obj,seed,V,Q,phi,previousV,Vaas,Qaas,currentQ)
			%Stimulation control flags
			stimFlags=zeros(1,14);
			backgroundNoiseFlags=zeros(1,3);
            excitabilityNoiseFlags=zeros(1,3);
            excitabilityNoiseiFlags=zeros(1,3);
            whiteNoise=1;
			stimFlags(2)=1;
			%Store flag
            storeFlag=0;
			...
			for n=1:ceil(obj.finalTime*1/obj.h)
                ...
                [current,currentQ,currentPhi,delay,nstore,samplingFlag,storeFlag]=obj.indexShifting(n,currentQ,nstore);
				%Generate Noise
				[backgroundNoise,backgroundNoiseFlags]=obj.stimulator.generateBackgroundNoise(backgroundNoiseFlags);
                [excitabilityNoise,excitabilityNoiseFlags]=obj.stimulator.generateNoise(obj.model.muE,obj.model.SDE,whiteNoise,excitabilityNoiseFlags);
                [excitabilityNoisei,excitabilityNoiseiFlags]=obj.stimulator.generateNoise(obj.model.muE,obj.model.SDE,whiteNoise,excitabilityNoiseiFlags);
				%Generate the stim
				[obj.stimulator,stim,marker,stimFlags]=obj.stimulator.generateStim(n,stimFlags);
				%Set the input
			    phi(obj.indexesN,current)=stim+backgroundNoise;
			    ...
			    %Time in seconds
                currentTime=n*obj.h; %in samples
			    [V,Q,phi,previousV,Vaas,Qaas]=obj.stepAPE(V,Q,phi,excitabilityNoise,excitabilityNoisei,previousV,Vaas,Qaas,current,currentQ,currentPhi,delay,currentTime);
				...
				if obj.plasticityFlag==2
                 ...
                 if mod(currentTime,5)==0
                    fprintf('Transcurred: %.2f seconds of %s at %s \n',currentTime,obj.monitor.filename,datestr(now,'HH:MM:SS DD-mm-YY'));
                    fprintf('Phi_e %.6e  V_e %.6e  Q_m %.6e Stim %d \n',phi(1,current),V(1,1),Qaas(2),stimFlags(2));
                 end
			end
		...
		%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        %% Principal encapsulating method including presolve %(PREFERABLY use only for small duration tests)
	...
	%% Presolver method, 
		...
		function[steadyV,steadyQ,steadyPhi,steadyPreviousV,Vaas,Qaas,current,currentQ,currentPhi,delay]=presolveAAS(obj,stimulator,seed)
            ...
            rng(seed);            
            backgroundNoise=stimulator.noiseMean+stimulator.noiseSD*randn(obj.indexMax/5,ceil(1/obj.h)*obj.previousTime);
            excitabilityNoise=obj.model.muE+obj.model.SDE*randn(obj.indexMax/5,ceil(1/obj.h)*obj.previousTime);
            excitabilityNoisei=obj.model.muE+obj.model.SDE*randn(obj.indexMax/5,ceil(1/obj.h)*obj.previousTime);
            meanV=0;
            meanphi=0;
            minv=1;
            meanQ=0;
            ...
            for n=1:ceil(1/obj.h*obj.previousTime)
				...
				phi(obj.indexesN,current)=backgroundNoise(:,n);
				%Simulation step
                [V,Q,phi,previousV,Vaas,Qaas]=obj.stepAPE(V,Q,phi,excitabilityNoise(:,n),excitabilityNoisei(:,n),previousV,Vaas,Qaas,current,currentQ,currentPhi,delay,n*obj.h);
                %Save values and search for the minimum value
                if n>timeSkip
					%Means of coritcal population
					meanV=meanV+mean(V(obj.indexesE,1));
	...
	%% Step
        ...
        %%%%%%%%%%%%%%%%%%%%%%%
        %StepAAS
        %Kernel of the simulations with the addition of the AAS dynamics
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        %Step APE
        %Kernel of the simulations with AAS dynamics, plasticity in the connection strengths and additional excitability in the cortex
        function [V,Q,phi,previousV,Vaas,Qaas]=stepAPE(obj,V,Q,phi,excitabilityNoise,excitabilityNoisei,previousV,Vaas,Qaas,current,currentQ,currentPhi,delay,time)
            %Dendrytic Voltages, taking in account the thalamocortical-delay 
            phi_current=phi(:,current)+Qaas(2)*obj.model.stencilAAS;       
            %vAux=obj.model.connectionMatrixNodelay*phi_current+obj.model.connectionMatrixDelay*phi(:,delay);
            vAux=prodsum(phi_current,obj.model.connectionMatrixNodelay,phi(:,delay),obj.model.connectionMatrixDelay);
            vAux(obj.indexesE)=vAux(obj.indexesE)+excitabilityNoise;
            vAux(obj.indexesI)=vAux(obj.indexesI)+excitabilityNoisei;
            D=obj.modelAAS.sleepDrive(time,Vaas(3));
            u=obj.modelAAS.vector_u(Qaas,D,obj.Ach);
