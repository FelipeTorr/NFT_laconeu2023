classdef Stimulator
    ...
    methods
        ...
        function obj=Stimulator(mode,config)
            ...
            obj.envelope_tol_count=0;
			obj.envelope_accum=0;
			obj.envelope_flag=0;
			obj.envelope_tol=5e-4;
            obj.envelope_threshold=5e-4;
        ...
        function [noise,flags]=generateBackgroundNoise(obj,flags)	
        ...
        function [noise,flags]=generateNoise(obj,noiseMean,noiseSD,noiseColor,flags) 
            %%noiseColor=0 zero; 1 White Gaussian; 2 Pink Gaussian temporal, uniform spatial; 3 Pink Gaussian temporal, Gaussian spatial;        
            noise=zeros(obj.Nx*obj.Ny,1);
            if noiseColor==0
                flags=flags;
            elseif noiseColor==1
                noise=noiseMean+noiseSD*randn(obj.Nx*obj.Ny,1);
            elseif  noiseColor==2
                [noise_node,randgen]=util.pinknoise(flags);
                flags=randgen;
                for k=1:obj.Nx*obj.Ny
                    noise_out=noiseMean+noiseSD*noise_node*rand(1,1);
                    noise(k)=noise_out;
                end
            elseif obj.noiseColor==3
                [noise_node,randgen]=util.pinknoise(flags);
                flags=randgen;
                noise=noiseMean+noiseSD*noise_node*randn(256,1);
            end
        end
        ...
        function [obj,shape_now,marker,stimulationFlags]=phaseLockSONgo(obj,flags,n)
            ...
            if obj.envelope_flag==0 && obj.envelope_count>0 && n>nsamples
                ...
                obj.envelope_tol=obj.envelope_accum/obj.envelope_tol_count;
                obj.envelope_flag=1;
            ...
            elseif abs(obj.x_now(1)-obj.x_now(2))<=obj.envelope_tol && obj.x_now(1)>=obj.x_now(2)
                ...
                if obj.x_now(3)<obj.peak_value && obj.x_now(1)>obj.peak_value 
                    ...
                    if n<nsamples
                        ...
                        obj.envelope_tol_count=obj.envelope_tol_count+1;
                    end
                else
        ...
        %Phase stimulation
         function [obj,shape_now,marker,stimulationFlags]=phaseLock(obj,flags,n)
            ...
            obj.previous_delta(1)=V-obj.meanPhi;
            shape_now=0;
            nsamples=10000; %1 second para h=1e-4
            %filtering
            [filtro_delta,obj.previous_delta_filter]=util.filtrar(obj.numDelta,obj.denDelta,obj.previous_delta,obj.previous_delta_filter); %filtrar
            obj.x_now(2:3)=obj.x_now(1:2);
            obj.y_now(2:3)=obj.y_now(1:2);
            ...
            %%%Envelope method
            %Set a better tolerance
            if obj.envelope_flag==0 && obj.envelope_tol_count>0 && n>nsamples
                %Calculate a new tolerance after nsamples of simulated data
                obj.envelope_tol=obj.envelope_accum/obj.envelope_count;
                obj.envelope_flag=1;
            end
            %First amplitude threshold, the peak amplitude must be at least equal to obj.envelope_tol (5e-4 at the start)
			if obj.x_now(1)<obj.envelope_tol
				%Set the search for peak value to 0.01 if the signal is below the tolerance
				obj.peak_value=obj.ENVELOPE_OFFSET;
			elseif abs(obj.x_now(1)-obj.x_now(2))<=obj.envelope_tol && obj.x_now(1)>=obj.x_now(2)
            ...
            if obj.envelope_count<nsamples
                obj.envelope_avg=obj.envelope_avg+envolvente; %Envelope average accumulator
            else
                avg=obj.envelope_avg/obj.envelope_count;
                obj.envelope_threshold=0.1*avg; %10% the previous envelope average
            ...
            %threshold of stimulus application
            if envolvente<2*obj.envelope_threshold;
                flag_hold=1;
            end
			%Save the actual envelope
			obj.previous_envelope=envolvente;
			...
			%Envelope detection method for the shifted-phase signal
			
			if obj.y_now(1)<obj.envelope_tol_shift
				obj.peak_value_shift=obj.envelope_tol_shift;
			elseif abs(obj.y_now(1)-obj.y_now(2))<=obj.envelope_tol_shift && (obj.y_now(1)-obj.y_now(2))>=0
				if obj.y_now(3)<obj.peak_value_shift && obj.y_now(1)>obj.peak_value_shift
				    obj.envelope_wait_shift=true;	
				else
			...
			if (flag_search==1 && obj.phase>=obj.targetPhase-EPSILON_ANGLE && obj.phase<=obj.targetPhase+EPSILON_ANGLE)		        
				if flag_hold==0 && flag_shot==0
                    ...
                    %Set the pulse marker, which also is the flag for stimulation
                    marker=1;
                    %fprintf('Stim: %d \n',flags(2)+1);
                    %Add the stim counter
                    flags(2)=flags(2)+1;
