...
def normalisedDistance(x,y):
    x_n=x/np.sum(x)
    y_n=y/np.sum(y)
    distance=np.sqrt(np.sum((x_n-y_n)**2))
    return distance

def angularSimilarity(x):
...
def SPSOcorrelate(meanSObaseline,SP_delay,fs=100):
    histSP,tbin=np.histogram(SP_delay,bins=np.arange(-2,2+1/fs,1/fs),density=True)
    lowFreq=0.5
    highFreq=1.25
    b,a=signal.cheby1(4,1e-6,[lowFreq/(fs/2), highFreq/(fs/2)],'bandpass')
    filteredSP=signal.filtfilt(b,a,histSP)
    SOSP=signal.correlate(meanSObaseline-np.mean(meanSObaseline),filteredSP-np.mean(filteredSP))
    SOSP=SOSP/(np.sqrt(np.sum(meanSObaseline**2))*np.sqrt(np.sum(filteredSP**2)))
    #SOSP=signal.correlate(meanSObaseline,histSP)
    return SOSP

def argwhereminmax(x):
    minimo=np.argwhere(x==np.min(x))[0][0]
    maximo=np.argwhere(x==np.max(x))[0][0]
    return minimo, maximo

def correlate2D(x,y):
    return np.matnul(x,y.T)
