...
def detectAPESOBaseline(data,neg_threshold=-40e-6,p2p_threshold=80e-6,fs=100,lengthcount=90000):
    shapeData=np.shape(data)
    SOdetection=np.zeros_like(data)
    SOsegmentation=np.zeros_like(data)
    SOp2p=[]
    ...
    zeros_down=[]
    SOs=[]
    SOthreshold=np.ones((len(filteredSignal),1))
    zeros_prev=np.zeros((lenTime//100,),dtype=int)
    zeros_post=np.zeros((lenTime//100,),dtype=int)
    zero_post=0
    good_so=[]
    n_previous=0
    previousLengthcount=0
    for n in range(lenTime):
        #Detect the zero-crosses from up to down in the filtered signal
        if n>0 and filteredSignal[n-1]>0 and filteredSignal[n]<0:
            zeros_down.append(n)
    ...
    #Threshold
    SOp2p=np.array(SOp2p)
    SOp2ptime=np.array(SOp2ptime)
    n1_end=SOp2ptime[len(SOp2ptime)-1]
    for i in range(len(filteredSignal)):
        if i>SOp2ptime[0] and i<SOp2ptime[len(SOp2ptime)-1]:
            if i<len(filteredSignal)-lengthcount:
                n_start=np.argwhere(SOp2ptime>=i)[0][0]
                n2_end=np.argwhere(SOp2ptime>=i+lengthcount)
                if n2_end.size>0: 
                    n_end=n2_end[0][0]
                else:
                    n_end=n1_end
                ...
                n_start=np.argwhere(SOp2ptime>=i)[0][0]
                n_end=n1_end
            SOthreshold[i]=1.5*np.mean(SOp2p[n_start:n_end])
    SOthreshold[0:SOp2ptime[0]+1]=SOthreshold[SOp2ptime[1]]
    SOthreshold[SOp2ptime[len(SOp2ptime)-1]::]=SOthreshold[SOp2ptime[len(SOp2ptime)-1]-1]
    #Refining
    nn=0
    if count>0:
        for n in range(1,len(SOp2p)+1):
            if SOp2p[n-1]<SOthreshold[zeros_prev[n]]:
                SOsegmentation[zeros_prev[n]:zeros_post[n]]=0
            else:
                good_so.append(n)
        for n in good_so:
    ...
    count=len(SOs)
    return SOdetection, SOsegmentation, SOs,count, SOthreshold

def detectAPESO(data,SOthreshold,neg_threshold=-40e-6,p2p_threshold=80e-6,fs=100):
    shapeData=np.shape(data)
    SOdetection=np.zeros_like(data)
    SOsegmentation=np.zeros_like(data)
    SOp2p=[]
    ...
    count=len(SOs)
    return SOdetection, SOsegmentation, SOs,count
