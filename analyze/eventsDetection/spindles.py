...
def detectsegmentSpindles(meanData,fs=100,threshold=0.26,gap_max=0.1):
    ...
    filtered=signal.filtfilt(b,a,meanData)
    rms=np.zeros((len(filtered),1))
    smoothed_rms=np.ones((len(filtered),1))
    spindle_detection=np.zeros((len(filtered),1))
    ...
    for n in range(timeWindow,len(filtered)):
        ...
        if n>timeWindow:
            ...
            if flag_filt==0:
                ...
                smoothed_rms[n]=smooth_rms[timeWindow-1]
            # if np.min(smoothed_rms[0:n])<=threshold:
            if smoothed_rms[n]>=threshold:
                if hold==0:
                    start=n
                accum+=1
    ...
    for n in range(timeWindow,len(filtered)):
        ...
        if n>timeWindow:
            ...
            if flag_filt==0:
                ...
                smoothed_rms[n]=smooth_rms[timeWindow-1]
    return np.nanmean(smoothed_rms)

def stdRMS(meanData,fs=100,lowFrequency=9,highFrequency=16):
    b,a=signal.cheby1(4,1e-6,[lowFrequency/(fs/2), highFrequency/(fs/2)],'bandpass')
    filtered=signal.filtfilt(b,a,meanData)
    rms=np.zeros((len(filtered),1))
    smoothed_rms=np.zeros((len(filtered),1))
...
def detectAPESpindlesBaseline(meanData,fs=100,gap_min=0.1,timelong=900):
    b,a=signal.cheby1(4,1e-6,[9/(fs/2), 16/(fs/2)],'bandpass')
    filtered=signal.filtfilt(b,a,meanData)
    rms=np.zeros((len(filtered),1))
    smoothed_rms=np.ones((len(filtered),1))
    threshold=np.ones((len(filtered),1))*np.nanmean(filtered)
    spindle_detection=np.zeros((len(filtered),1))
    
    spindle_segmentation=np.zeros((len(filtered),1))
    timeWindow=int(np.ceil(0.2*fs));
    longtimeWindow=int(np.ceil(timelong*fs));
    #Spindle Characterisitics
    tmin=0.49*fs
    tmax=2.01*fs
    gap_min=gap_min*fs; #Inter-spindle refractoriness
    hold=0
    start=0
    accum=0
    gap=0
    ...
    for n in range(timeWindow,len(filtered)):
        ...
        if n<(len(filtered)-longtimeWindow):
            threshold[n]=1.5*np.nanstd(smoothed_rms[n:longtimeWindow+n])+np.nanmean(smoothed_rms[n:longtimeWindow+n])
        else:
            threshold[n]=1.5*np.nanstd(smoothed_rms[n::])+np.nanmean(smoothed_rms[n::])
        if smoothed_rms[n]>=threshold[n]:
            if hold==0:
                start=n
            accum+=1
            hold=1
            ...
            if gap>gap_min:
                ...
                if accum>=tmin and accum<=tmax:
                    ...
                    spindle_segmentation[start:n]=count
                accum=0
    threshold[0:timeWindow]=threshold[timeWindow]            
    return filtered,smoothed_rms,spindle_detection,spindle_segmentation,count,threshold


def detectAPESpindles(meanData,threshold,fs=100,gap_min=0.1):
    b,a=signal.cheby1(4,1e-6,[9/(fs/2), 16/(fs/2)],'bandpass')
    filtered=signal.filtfilt(b,a,meanData)
    rms=np.zeros((len(filtered),1))
    smoothed_rms=np.ones((len(filtered),1))
